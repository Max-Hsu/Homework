...
# as a way of indicating that this can go on multiple lines.
	...
	def toAset(self):
		return {1 if self.__tpos in W else 2 } if (self.__tpos in B or self.__tpos in W) else set()
...
#	positions to determine our comparison answers. 
	...
	def __eq__(self,num=None):
		#print("eq called ")
		#print("eq check",self.toAset,{num}-{None},self.toAset() == ({num}-{None}))
		return self.toAset() == ({num}-{None})
	...
	def __gt__(self,num=None):
		#print("called")
		#print("gt check",self.toAset,{num}-{None},self.toAset() == ({num}-{None}))
		return self.toAset() > ({num}-{None})
	def __contains__(self,dicta):
		return self.__tpos in dicta
	# These methods define the following operators: <, <=, ==, !=, >=,
	# >, and "in".
	...
	def remove(self):
		try:
			...
			return None
# The idea here is a little odd. We don't remove this pos
...
#  4. If there is no piece at this position, return: None.
	...
	def __ilshift__(self,pos_):
		#print(self.toAset(),type(self.toAset()))
		#print(pos_.toAset(),type(pos_.toAset()))
		assert pos_ > None
		#print(next(iter(self.toAset())) if ( self.toAset() != set() ) else None)
		assert pos_ != (next(iter(self.toAset())) if ( self.toAset() != set() ) else None)
		color = pos_.toAset()
		check = pos_.remove()
		assert (check != None)
		to_position_check = self.remove()
		if check.color == 1:
			W[self.__tpos]=check.symbol
		elif check.color == 2:
			B[self.__tpos]=check.symbol
		if to_position_check != None:
			if to_position_check.symbol == "♛":
				return True
			else:
				return False
		else:
			return False
		##how to know if i had the king?

# This method defines the <<= operator (logical shift left).
