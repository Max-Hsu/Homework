...
def GetAMove(WorB=None):
	if(WorB is None):
		inputs=input("White\nMove:")
		#print(inputs)
		IsLegal,Position,ToPosition,killa=Legal(True,*inputs)
		while(IsLegal==False):
			inputs=input("Wrong Input\nWhite\nMove:")
			IsLegal,Position,ToPosition,killa=Legal(True,*inputs)
		WorB=[]
		WorB.append("white")
		return WorB,Position,ToPosition,True,killa
	else:
		inputs=input("Black\nMove:")
		IsLegal,Position,ToPosition,killa=Legal(False,*inputs)
		while(IsLegal==False):
			inputs=input("Wrong Input\nBlack\nMove:")
			IsLegal,Position,ToPosition,killa=Legal(False,*inputs)
		return None,Position,ToPosition,False,killa
...
def Legal(WorB,*inputs):
	...
	if(SyntacticallyLegal(inputs)):
		SemantLegal,Position,killa=SemanticallyLegal(WorB,inputs)
		if(SemantLegal==True):
			return True,Position,ToPosition,killa
	return False,[],[],[]
...
def SemanticallyLegal(WorB,inputs):
	...
	return checkAfterInterpret,listOfChangePosition,killa
	"""
	elif(lenOfInputs==3 and checkRange_alphabet(inputs[0]) and checkRange_number(inputs[1]) and inputs[2]=='x'):
		return True
	"""
...
def PlayGame():
	...
	WorB=None
	while(True):
		WorB,Position,ToPosition,TrueColor,killa=GetAMove(WorB)
		...
		if(TrueColor==True):
		...
		elif(TrueColor==False and killa==True):
			#print("black is killing")
			PositionComplex=complex(Position[0],Position[1])
			NewPositionComplex=complex(ToPosition[0],ToPosition[1])
			#print(len(W))
			#print(W)
			for runComplex in range(len(W)):
				#print(runComplex)
				if(W[runComplex]==NewPositionComplex):
					W.pop(runComplex)
					break
			#print(len(W))
			#print(W)
			...
			print("aa",PositionComplex,NewPositionComplex)
			for runComplex in range(len(W)):
				print(W[runComplex])
			'''
			'''
