...
int L_internal_buttons;
int R_internal_buttons;
...
float L_floor 	=	1;	//1
int L_state		=	0;  //0 for closed , 1 for opening , 2 for full open , 3 for closing
//int L_ele_prev_floor = -1;
int L_Direction = -1;	//0 for up , 1 for down
...
int main (void)  {                             /* main entry for program      */
	...
	for(i=0 ; i<16;i++){
  	...
  	while (1)  { 
		...
		*/
		//"i" was 1
		request_all_from_button[0] = 0;
		for(i=0 ; i<16 ; i++){
			//request_all_from_button[i] = (user_request[i]|button_on[i])?1:0;
			request_all_from_button[i] = (user_request[i]|state_button[i])?1:0;
		}
		L_floor_into_int_by_mul2 = L_floor*2;//well the switch need int so i multiply the L_floor by twp
					...
					if(request_all_from_button[13]){
						...
						switch (L_state){
							...
							case 2:
								...
								break;
							case 3:
								if(request_all_from_button[12]){
									L_elevator_closed();//change L_state to 0(closed)
								}
									save_request(L_floor);//save the requests;
								break;
						}
