...
cwr_edge_handles *cwr_edge_handles_10000;
sc_method_handle  cwr_edge_handles_10000_0;
void *  cwr_edge_handles_10000_0_host;
...
int cwr_optimizer_impl::register_handles(cwr_edge_handles& handles, int i) {
  ...
  case 0:
    ...
    cwr_edge_handles_0_0_host = reinterpret_cast<char *>(handles.m_clkonly_handles[0]->get_semantics_host()) + -176;
    break;
  case 1:
    cwr_edge_handles_10000 =  &handles;
    cwr_edge_handles_10000_0 = handles.m_clkonly_handles[0];
    cwr_edge_handles_10000_0_host = reinterpret_cast<char *>(handles.m_clkonly_handles[0]->get_semantics_host()) + -176;
    break;
  default:
    return 1;
  }
...
l_2:
  if (loop_offset <= 20000ull) {
    assert (cwr_prev_duration_0 != 0);
    next_edge += 20000ull - loop_offset;
    *m_cwr_next_time = next_edge;
    if (next_edge <= end) {
      if (cwr_ctx->simulate_until(next_edge, incr_delta_ramp_up))
        goto edge_0;
...
edge_1:
  // process clock edges at time 10000
  if (! cwr_ctx->one_delta() ) return;
  cwr_ctx->negedge_update(cwr_clk0_0_0);
  cwr_ctx->make_delta_cb();

  {
    sim_ctx->set_curr_proc(cwr_edge_handles_10000_0);
    _ZN5Clock5do_itEv(cwr_edge_handles_10000_0_host );
    sim_ctx->reset_curr_proc();
  }
  *m_cwr_next_time = next_edge += time_0;
  if (! cwr_ctx->do_crunch2()) return;
