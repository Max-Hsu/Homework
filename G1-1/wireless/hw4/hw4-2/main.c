...
void * tester_clean_method(void * timea)
{
    ...
    while(alarm_status)
    {
        ...
        {
            decide_channel=rand()%good_channel_counter;
            status_trywait=sem_trywait(&semas[decide_channel]);
            //printf("good\n");
        ...
        {
            decide_channel=rand()%channel_clean;
            status_trywait=sem_trywait(&semas[decide_channel]);
            hop++;
            if(status_trywait==-1)
            {
...
void * tester_crowded_method(void * timea)
{
    ...
    while(alarm_status)
    {
        ...
        if(good_channel_counter!=0)
        {
            ...
            {
                int plus;
                plus=(rand()%channel_crowded)+channel_clean;
                if(sem_trywait(&semas[plus])==0)
                {
                    usleep(hopping_time);
                    sem_post(&semas[plus]);
        ...
        {
            decide_channel=rand()%channel_crowded+channel_clean;
            status_trywait=sem_trywait(&semas[decide_channel]);
            hop++;
            if(status_trywait==-1)
            {
...
int main()
{
    ...
    */
    ...
    for(int k=0; k<2; k++)
    {
        ...
        float bad_channel_value;
        bad_channel_value=(double)all_collision/all_hop;
        ...
        printf("method4 bad_channel %f  device: %d    after: %d sec \t total collision:%ld  \t total hop:%ld  \t probability: %lf\n",bad_channel_value,device_to_test[k],time_for_test,all_collision,all_hop,(double)all_collision/all_hop);
