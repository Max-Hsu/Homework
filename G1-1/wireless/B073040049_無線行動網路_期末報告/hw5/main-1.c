...
#define channel_clean 40 //80*0.7
#define channel_crowded 20 //80*0.3
    ...
    while(alarm_status)
    {
        ...
        if(bad_channel[decide_channel]==1)
        {
            ...
            for(int q=0; q<2; q++)
            {
                ...
                if(minusa<0)
                {
                    minusa+=channel_clean;
                }
                //printf("%d\n",decide_channel);
                //printf("clean plus:%d minus:%d\n",plus,minusa);
    ...
    while(alarm_status)
    {
        ...
        if(bad_channel[decide_channel]==1)
        {
            ...
            for(int q=0; q<2; q++)
            {
                offseta++;
                int plus=decide_channel+offseta;
                if(plus>=channel_crowded+channel_clean)
                {
                    plus-=channel_crowded;
                }
                int minusa=decide_channel-offseta;
                if(minusa<channel_clean)
                {
                    minusa+=channel_crowded;
                }
                //printf("%d\n",decide_channel);
                //printf("crowded plus:%d minus:%d\n",plus,minusa);
                if(bad_channel[plus]==0)
                {
                    if(sem_trywait(&semas[plus])==0)
                    {
    ...
    for(int k=0; k<2; k++)
    {
        for(float bad_channel_value=0.1; bad_channel_value<1; bad_channel_value+=0.1)
        {
            ...
            for(int i=0;i<device_now;i++){
                device_position[i][0]=rand()%500;
                device_position[i][1]=rand()%500;
                if((device_position[i][0]<edge)&&(device_position[i][1]<edge)){
            ...
            for(int i=0; i<channel_clean+channel_crowded; i++)
            {
            ...
            printf("method1 bad_channel %f  device: %d    after: %d sec \t total collision:%ld  \t total hop:%ld  \t probability: %lf\n",bad_channel_value,device_to_test[k],time_for_test,all_collision,all_hop,(double)all_collision/all_hop);
